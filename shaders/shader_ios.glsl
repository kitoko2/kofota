...
void main() {
    ...
    vec2 distortedUv = vec2(normalizedUv.x + (normalizedUv.x - 0.5) * pow(normalizedUv.y, 6.0) * timeCu * 0.1, (normalizedUv.y * (normalizedUv.y * pow((1.0 - timeSq * 0.01), 8.0)) + (1.0 - normalizedUv.y) * normalizedUv.y));
    ...
    if(limitedTime >= 1.0) {
        ...
        explosionLight = smoothstep(0.09, 0.05, explosionLight) *
            ...
            (normalizedUv.y + 0.05);
        explosionShift = vec2(-EXPLOSION_LIGHT_SHIFT * explosionLight * normalizedUv.x, -4.0 * explosionLight * (normalizedUv.y - EXPLOSION_LIGHT_Y_OFFSET)) * 0.1;
        ...
        explosionLight2 = smoothstep(0.09, 0.05, explosionLight2) *
            ...
            (normalizedUv.y + 0.05);
        explosionShift += vec2(-EXPLOSION_LIGHT_2_SHIFT * explosionLight2 * normalizedUv.x, -4.0 * explosionLight2 * (normalizedUv.y - EXPLOSION_LIGHT_2_Y_OFFSET)) * -0.02;
    }
